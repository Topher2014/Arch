...
# ID: 9d023eba-b4c6-492d-b76d-04f612093b9b
...
echo -e "Name: archConfigServerEncrypted"
echo -e "ID: 9d023eba-b4c6-492d-b76d-04f612093b9b"
echo -e "Description: A spell to configure a new Arch instance"
echo -e "Identity: topherludlow@protonmail.com"
echo -e "Converging 40 resources..."
converge=''
cycle=0
converged="$(printf "\xE2\x98\xB1%.0s "  $(seq 40))"
echo -en "\033[0m"

while [ "$converge" != "$converged" ] && [ $cycle -le 40 ]
  do
    converge=''
...
" | tee -a /etc/hosts)
    ...
    if (systemctl is-active dhcpcd.service > /dev/null)
    then
      c_exit=0
      echo -en "\033[0;32m"
      echo -e "  \xE2\x9C\x93 dhcpcd.service should be started"
      echo -en "\033[0m"
      u_exit=1
    else
      c_exit=1
      echo -en "\033[0;31m"
      echo -e "  \xE2\x9C\x97 dhcpcd.service should be started"
      echo -en "\033[0m"
      u_out=$(systemctl start dhcpcd.service)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
    ...
    if (grep -qw "llmServer" /etc/hostname > /dev/null)
    then
      c_exit=0
      echo -en "\033[0;32m"
      echo -e "  \xE2\x9C\x93 Hostname should be set to llmServer"
      echo -en "\033[0m"
      u_exit=1
    else
      c_exit=1
      echo -en "\033[0;31m"
      echo -e "  \xE2\x9C\x97 Hostname should be set to llmServer"
      echo -en "\033[0m"
      u_out=$(echo "llmServer" | tee /etc/hostname)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
migrateold=false" > ~/.config/user-dirs.conf')
    ...
    if (systemctl is-enabled dhcpcd.service > /dev/null)
    then
      c_exit=0
      echo -en "\033[0;32m"
      echo -e "  \xE2\x9C\x93 dhcpcd.service should be enabled"
      echo -en "\033[0m"
      u_exit=1
    else
      c_exit=1
      echo -en "\033[0;31m"
      echo -e "  \xE2\x9C\x97 dhcpcd.service should be enabled"
      echo -en "\033[0m"
      u_out=$(systemctl enable dhcpcd.service)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
end_time="$(date -u +%s)"
  ...
  echo -en "\033[1;33m"
  echo -e "* The Spellcaster free tier includes up to 10 trigraphs. You are at 40 trigraphs and should sign up for an individual plan here https://spellcaster.sh/pricing.\n"
  echo -en "\033[0m"
