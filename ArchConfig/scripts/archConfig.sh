...
# Name: archConfig
# ID: 64cd7251-68ae-4252-b778-03a825015b9b
...
echo -e "ID: 64cd7251-68ae-4252-b778-03a825015b9b"
echo -e "Description: A spell to configure a new Arch instance"
echo -e "Identity: topherludlow@protonmail.com"
echo -e "Converging 64 resources..."
converge=''
cycle=0
converged="$(printf "\xE2\x98\xB1%.0s "  $(seq 64))"
echo -en "\033[0m"

while [ "$converge" != "$converged" ] && [ $cycle -le 64 ]
  do
    converge=''
    ...
    then
      ...
      echo -en "\033[0m"
      u_out=$(find /tmp/visual-studio-code-bin -name "VSCode-*.pkg.tar.zst" -not -name "*debug*" | xargs pacman -U --noconfirm)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
" | tee -a /etc/hosts)
    ...
    then
      ...
      echo -en "\033[0m"
      u_out=$(find /tmp/yay -name "spotify-*.pkg.tar.zst" -not -name "*debug*" | xargs pacman -U --noconfirm)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
end_time="$(date -u +%s)"
  ...
  echo -en "\033[1;33m"
  echo -e "* The Spellcaster free tier includes up to 10 trigraphs. You are at 64 trigraphs and should sign up for an individual plan here https://spellcaster.sh/pricing.\n"
  echo -en "\033[0m"
