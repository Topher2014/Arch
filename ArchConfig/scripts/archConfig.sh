...
# Name: archConfig
# ID: f81585df-2668-44e0-af97-eb236565a44e
...
echo -e "ID: f81585df-2668-44e0-af97-eb236565a44e"
echo -e "Description: A spell to configure a new Arch instance"
echo -e "Identity: topherludlow@protonmail.com"
echo -e "Converging 15 resources..."
converge=''
cycle=0
converged="$(printf "\xE2\x98\xB1%.0s "  $(seq 15))"
echo -en "\033[0m"

while [ "$converge" != "$converged" ] && [ $cycle -le 15 ]
  do
    converge=''
...
" | sudo tee -a /etc/hosts)
    ...
    if (test -f /tmp/yay/PKGBUILD && grep "makedepends" /tmp/yay/PKGBUILD | sed -e "s/^makedepends=(//" -e "s/)$//" | tr "'" " " | awk '{print $1}' | sed 's/>.*$//' | xargs pacman -Q > /dev/null)
    then
      c_exit=0
      echo -en "\033[0;32m"
      ...
      echo -en "\033[0m"
      u_out=$(grep "makedepends" /tmp/yay/PKGBUILD | sed -e "s/^makedepends=(//" -e "s/)$//" | tr "'" " " | awk '{print $1}' | sed 's/>.*$//' | xargs pacman -S --noconfirm)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
end_time="$(date -u +%s)"
  ...
  echo -en "\033[1;33m"
  echo -e "* The Spellcaster free tier includes up to 10 trigraphs. You are at 15 trigraphs and should sign up for an individual plan here https://spellcaster.sh/pricing.\n"
  echo -en "\033[0m"
