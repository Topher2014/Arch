...
# Name: archVM
# ID: 373c56c2-e3da-4b03-b243-37664a2b68e9
...
echo -e "Name: archVM"
echo -e "ID: 373c56c2-e3da-4b03-b243-37664a2b68e9"
...
converge=''
cycle=0
converged="$(printf "\xE2\x98\xB1%.0s "  $(seq 14))"
echo -en "\033[0m"
...
while [ "$converge" != "$converged" ] && [ $cycle -le 14 ]
  do
    ...
    then
      ...
      echo -en "\033[0m"
      u_out=$(sudo qemu-img create -F qcow2 -b /var/lib/libvirt/images/Arch-Linux-x86_64-cloudimg.qcow2 -f qcow2 /var/lib/libvirt/images/${VM:-arch-vm}.qcow2 ${VM_STORAGE:-100G})
      u_exit=$?
    fi
    r_exit=$?
    ...
    if (pacman -Q wget > /dev/null)
    then
      c_exit=0
      echo -en "\033[0;32m"
      echo -e "  \xE2\x9C\x93 wget should be installed"
      echo -en "\033[0m"
      u_exit=1
    else
      c_exit=1
      echo -en "\033[0;31m"
      echo -e "  \xE2\x9C\x97 wget should be installed"
      echo -en "\033[0m"
      u_out=$(sudo pacman -S wget --noconfirm)
      u_exit=$?
    fi
    r_exit=$?
    # echo "$r_exit$c_exit$u_exit"
...
end_time="$(date -u +%s)"
  ...
  echo -en "\033[1;33m"
  echo -e "* The Spellcaster free tier includes up to 10 trigraphs. You are at 14 trigraphs and should sign up for an individual plan here https://spellcaster.sh/pricing.\n"
  echo -en "\033[0m"
